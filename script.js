...
// Không sử dụng câu hỏi dạng viết lại câu trong phiên bản này.
QUESTIONS = QUESTIONS.filter(q => q.type !== 'rewrite');
...
let log = [];
/** @type {Array<{answered:boolean, p:number, m:number}>} Persistent answer state per question */
let questionState = QUESTIONS.map(() => ({ answered: false, p: 0, m: 0 }));
...
function recordScore(q, points) {
  const st = questionState[ci];
  if (!st || st.answered) return;
  st.answered = true;
  st.p = points;
  st.m = q.pt;
  ans = true;
  sc = questionState.reduce((sum, x) => sum + (x.answered ? x.p : 0), 0);
  updHUD();
  log = questionState
    .map((x, i) => x.answered ? { tag: QUESTIONS[i].tag, p: x.p, m: x.m } : null)
    .filter(Boolean);
}

function appendQuestionNav(card) {
  const nav = document.createElement('div');
  nav.className = 'q-nav btn-row';

  const prev = document.createElement('button');
  prev.className = 'btn q-prev';
  prev.textContent = '← Quay lại';
  prev.disabled = ci === 0;
  prev.onclick = () => { if (ci > 0) { ci--; render(); } };

  const next = document.createElement('button');
  next.className = 'btn primary q-next';
  next.textContent = ci < QUESTIONS.length - 1 ? 'Tiếp theo →' : 'Xem kết quả →';
  next.disabled = !(questionState[ci]?.answered || ans);
  next.onclick = () => {
    if (next.disabled) return;
    if (ci < QUESTIONS.length - 1) { ci++; render(); }
    else { renderResult(); }
  };
...
function render() {
  ...
  ans = questionState[ci]?.answered || false;
  fe = 0;
  updHUD();
...
function renderFind(card, q) {
  ...
  skipBtn.onclick = () => {
    ...
    recordScore(q, p);
    updHUD();
    doc.querySelectorAll('.err-span:not(.found)').forEach(s => { s.classList.add('found'); s.title = 'Đáp án đúng'; });
    const msg = p > 0
...
function renderResult() {
  ...
  let bdHtml = '';
  SECS.forEach(s => {
    const secLogs = questionState
      .map((entry, i) => ({ ...entry, qi: i }))
      .filter(entry => entry.answered && QUESTIONS[entry.qi].sec === s.sec);
    if (!secLogs.length) return;
    bdHtml += `<div class="bd-section">${s.label}</div>`;
    secLogs.forEach(entry => {
      bdHtml += `<div class="bd-row">
        <span class="label">${QUESTIONS[entry.qi].tag}</span>
  ...
  $('qa').innerHTML = `
    <div class="card result" id="result-card">
      ...
      <div class="btn-row result-actions" style="justify-content:center">
        <button class="btn primary" onclick="restart()">↺ Chơi lại từ đầu</button>
        <button class="btn" onclick="saveResultImage()">💾 Lưu kết quả</button>
        <button class="btn" onclick="shareResultImage()">📤 Chia sẻ kết quả</button>
      </div>
    </div>`;
}

/* ═══════════════════════════════════════════════════════
   SAVE / SHARE RESULT AS IMAGE
   ═══════════════════════════════════════════════════════ */
async function makeResultBlob() {
  const card = $('result-card');
  if (!card) return null;
  if (typeof html2canvas !== 'function') {
    alert('Chưa tải được thư viện tạo ảnh. Hãy kiểm tra kết nối Internet hoặc tải lại trang.');
    return null;
  }
...
async function saveResultImage() {
  const blob = await makeResultBlob();
  if (!blob) return;
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'ket-qua-game-soan-thao-van-ban.png';
...
async function shareResultImage() {
  const blob = await makeResultBlob();
  if (!blob) return;
  const file = new File([blob], 'ket-qua-game-soan-thao-van-ban.png', { type: 'image/png' });
  if (navigator.canShare && navigator.canShare({ files: [file] })) {
    await navigator.share({
      title: 'Kết quả game Soạn thảo Văn bản Hành chính',
      text: 'Kết quả luyện tập của tôi',
      files: [file],
    });
  } else {
    await saveResultImage();
    alert('Trình duyệt này chưa hỗ trợ chia sẻ trực tiếp. Ảnh kết quả đã được tải xuống để bạn chia sẻ thủ công.');
  }
}
...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
function restart() {
  ci = 0; sc = 0; fe = 0; ans = false; log = []; questionState = QUESTIONS.map(() => ({ answered: false, p: 0, m: 0 }));
  render();
}
